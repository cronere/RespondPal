...
- VARY YOUR SENTENCE STRUCTURE AND OPENING ACROSS DIFFERENT FINDINGS IN THIS RESPONSE. You are generating many rewrites in one continuous response, which creates a real risk of settling into a repeated template — the same opening ("Thank you so much —"), the same "our whole team" phrasing, the same well-wish closer, over and over. Before finalizing each rewrite, check it against the ones you've already written in THIS SAME response: if two rewrites share the same opening words, the same sentence rhythm, or the same closing well-wish, rewrite one of them with genuinely different phrasing and structure. A business owner reading all these rewrites together should not be able to tell they came from a template.
...
- AVOID THIS SENTENCE PATTERN ENTIRELY: "[general values or mission statement], and we're so glad that [verb]" — e.g. "creating a comfortable environment is something we care about, so it's wonderful to know that resonated" or "...and we're so glad that came through." This confirms the stated value was PERSONALLY EXPERIENCED by this specific reviewer, regardless of the verb (resonated, came through, rang true, held up — the verb changes, the disclosure doesn't). Banning individual verbs is chasing an infinite list; avoid the entire sentence shape instead.
- NEVER CONFIRM PHYSICAL RECOVERY, HEALING, OR IMPROVEMENT — especially critical for physical therapy, chiropractic, rehab, or any recovery-focused practice, where "I recovered" or "I feel better" IS the review content, making the warm, natural-sounding reply exactly the violation. "So glad to hear about the progress," "glad you're feeling better," "thrilled about your recovery" all directly confirm a specific health status change for this reviewer — arguably the most direct PHI disclosure possible. Acknowledge the REVIEW ITSELF, never the underlying health outcome, no matter how naturally the reply seems to call for it.
...
- Do NOT name specific staff members, providers, or specialists in a rewrite, even if the reviewer named them first and even if it feels warm to do so. Naming a provider in connection with this reviewer's care is our OWN top-priority signal for detecting a violation in original text — a rewrite cannot do the same thing and still be safe. This applies to EVERY name a provider might go by — first name, last name, nickname, or any alias — not just the specific name that appeared in the flagged text. If you know or infer that a provider goes by more than one name (e.g. "Dr. Luciana" and "Dr. Yacomotti" are the same person), substituting one for the other is NOT a fix — it's still naming them. The correct move is removing the provider reference entirely, not finding a different name for the same person. Use "our team" or "the whole practice" instead of ANY specific name. Safe: "Thank you so much — our team truly appreciates this!" Not safe: "Dr. Moss and Manda are wonderful" (still ties named providers to this reviewer's care) — and equally not safe to swap in "Dr. Alvarez" if Moss and Alvarez are the same person.
...
CONFIDENCE CALIBRATION — CONFIRMED PATIENT vs. NON-PATIENT: The enforcement cases cited in this report (New Vision Dental, Igbinadolor, Manasa Health Center) all involved practices disclosing information about their OWN CONFIRMED PATIENTS. If the review or response EXPLICITLY establishes the reviewer is NOT a patient of this business — e.g. "you are not a patient at our office," "a procedure you received at ANOTHER clinic," "we searched and couldn't find you," a reviewer clarifying they meant a different location — this is a MEANINGFULLY WEAKER legal case than a confirmed-patient disclosure, even if the response still discloses specific details. For these non-patient scenarios: (1) you may still flag "Privacy violation" if specifics are disclosed, but write the explanation with HEDGED language — "this is a closer legal question since the reviewer wasn't your patient, but the disclosure is still worth avoiding" — NEVER call it "textbook" or definitively cite the enforcement cases as direct precedent, since none of those cases involved a non-patient. (2) In the "summary" field, if the MOST PROMINENT or lead finding is a non-patient scenario, do not open with maximum-certainty language like "the most urgent issue is a critical HIPAA violation" — instead acknowledge the legal nuance honestly, e.g. "the most notable issue involves a caller who wasn't a patient here, which is a closer legal question than a typical violation, but the response still discloses more than necessary." Reserve full-confidence, precedent-citing language for scenarios where the reviewer IS clearly a patient of THIS business.
...
Respond ONLY with valid JSON in this exact structure, no other text:
{
  ...
  "findings": [
    {
      "review_summary": "star rating + 6-10 word summary of what the reviewer complained about, e.g. \'1★ — Client says appointment ran an hour late with no apology\'. CRITICAL — NEVER INVENT REVIEW CONTENT: if the review has NO written text (a bare star rating only, nothing else), you MUST write something like \'1★ — No written review text\' or \'1★ — Rating only, no comment left\'. Do NOT guess, infer, or fabricate a plausible-sounding complaint based on the business\'s response, the star rating alone, or anything else — this summary is presented to the business owner as a factual account of what a real customer wrote, and inventing content here is a serious accuracy failure, not a stylistic shortcut. If you are not looking at actual reviewer-written text in the raw input for this specific review, you have nothing to summarize and must say so plainly.",
...
3. Does it name a specific staff member or provider in connection with this reviewer's care — by ANY name they might go by (first name, last name, nickname, or alias), even if it's a different name than the one flagged in the original text? Substituting one name for another for the same person is still naming them.
